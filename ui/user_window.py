...
class UserApp:
    ...
    def create_widgets(self):
        # Загружаем данные о лотах
        self.data = load_json_data()
        self.items = self.data["items"]

        # Создаем кнопку для просмотра всех лотов
        self.view_items_button = tk.Button(self.root, text="Просмотр всех лотов", command=self.view_items)
        self.view_items_button.pack(pady=10)

        # Кнопка для выхода
        self.exit_button = tk.Button(self.root, text="Выход", command=self.logout)
    ...
    def view_items(self):
        ...
        tk.Button(items_window, text="Назад", command=items_window.destroy).pack(pady=10)
